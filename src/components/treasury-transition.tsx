...
const Navigate = createContext<((href: string, direction: Direction) => void) | null>(null);
...
export function TreasuryTransitionProvider({ children }: { children: ReactNode }) {
  const router = useRouter();
  const pathname = usePathname();
  const complete = useRef<(() => void) | null>(null);
  const active = useRef(false);
  useLayoutEffect(() => { complete.current?.(); complete.current = null; }, [pathname]);
  useEffect(() => () => { complete.current?.(); }, []);
  ...
  function navigate(href: string, direction: Direction) {
    ...
    const transition = document.startViewTransition(() => new Promise<void>((resolve) => {
      ...
      timer = setTimeout(() => { transition.skipTransition(); resolve(); }, 5000);
      router.push(href);
    }));
...
export function TreasuryLink({ direction = "left", href, onClick, ...props }: Omit<ComponentProps<typeof Link>, "href"> & { href: string; direction?: Direction }) {
  const navigate = useContext(Navigate);
  return <Link {...props} href={href} onClick={(event) => {
    onClick?.(event);
    if (!navigate || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || props.target === "_blank" || props.download) return;
