...
export function CustomerAccountHeaderButton({ locale }: { locale: "fa" | "en" }) {
  ...
  return (
    <Link
      ...
      aria-label={label}
      title={label}
      className="group relative flex h-10 items-center justify-center gap-2 overflow-hidden rounded-xl border border-white/10 bg-white/[0.045] px-2.5 text-white/65 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] transition duration-300 hover:border-[#dfbd68]/45 hover:bg-[#168461]/15 hover:text-[#f5dc9a] sm:h-11 sm:rounded-2xl sm:px-3"
    >
      ...
      <span className="relative z-10 hidden whitespace-nowrap text-[11px] font-medium lg:inline">
        {label}
      </span>
    </Link>
  );
