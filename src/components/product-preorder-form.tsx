...
export function ProductPreorderForm({ locale, slug, variantId }: { locale: "fa" | "en"; slug: string; variantId: string | null }) {
  ...
  if (receipt) return <div role="status" className="mt-8 rounded-2xl border border-[#d9b85f]/30 bg-[#061c15] p-6 leading-8"><p>{receipt.message}</p><a className="underline" href={`/${locale}/profile/preorders`}>{fa ? "پیگیری در حساب کاربری" : "Track in your account"}</a><p className="mt-4 text-sm">{fa ? "کد پیگیری درخواست:" : "Request reference:"}</p><p dir="ltr" className="break-all text-sm text-[#d9b85f]">{receipt.reference}</p><p className="mt-4 text-sm">{fa ? "کد را نگه دارید؛ پیگیری از طریق پشتیبانی الوریا و شمارهٔ ثبت‌شده انجام می‌شود." : "Keep this reference. Eloria support will follow up using your phone number."}</p></div>;
  return <form onSubmit={submit} className="mt-7 space-y-5 rounded-2xl border border-[#d9b85f]/25 bg-[#061c15]/90 p-5 sm:p-7">
    <p className="text-sm leading-8 text-[#d5c7a7]">{fa ? "این اثر فروخته شده است. درخواست شما برای بررسی ساخت دوباره ثبت می‌شود؛ پیش از پرداخت، زمان آماده‌سازی و قیمت نهایی را با شما هماهنگ می‌کنیم." : "This creation has been sold. Submit a request and we will confirm whether it can be made again, along with timing and final price, before any payment."}</p>
