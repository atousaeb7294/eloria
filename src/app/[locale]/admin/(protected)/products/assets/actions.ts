...
function publicAssetError(error: unknown, fallback: string): string {
  if (
    error instanceof AdminProductAssetError ||
...
function text(
  ...
  if (required && !value)
    throw new AdminProductAssetError("فیلدهای الزامی را تکمیل کنید.");
  if (value.length > max)
    throw new AdminProductAssetError("طول یکی از فیلدها بیش از حد مجاز است.");
  return value || null;
}
function integer(
  form: FormData,
...
function decimal(form: FormData, key: string): string | null {
  ...
  if (!/^\d+(\.\d+)?$/.test(value))
    throw new AdminProductAssetError("مقدار اعشاری معتبر نیست.");
  return value;
}
async function session() {
  if (!(await hasValidAdminSession()))
    throw new AdminProductAssetError("نشست مدیریت منقضی شده است.");
}
...
function refresh(productId: string, slug: string) {
  ...
  for (const locale of ["fa", "en"] as const) {
    ...
    revalidatePath(`/${locale}/world`);
    revalidatePath(`/${locale}/atelier`);
    revalidatePath(`/${locale}/story`);
...
export async function uploadAdminProductImagesAction(
  ...
  if (!files.length)
    redirect(
      messageUrl(locale, productId, "mediaError", "فایلی انتخاب نشده است."),
    );
  ...
  try {
    ...
    redirect(
      messageUrl(
        ...
        productId,
        "mediaError",
        publicAssetError(error, "آپلود ناموفق بود."),
      ),
    );
  }
  ...
  if (!isAllowedProductImageUrl(imageUrl)) {
    redirect(
      messageUrl(
        ...
        productId,
        "mediaError",
        "آدرس تصویر خارج از میزبان‌های مجاز است.",
      ),
    );
  }
...
export async function updateAdminProductImageAction(
  productId: string,
  imageId: string,
  localeValue: string,
  form: FormData,
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.productImage.update({
      where: { id: imageId },
      data: {
        altFa: text(form, "altFa", 240),
        altEn: text(form, "altEn", 240),
...
/**
 * Reuses an already validated image from this product's gallery. This lets the
 * editor place real character and scene artwork in the Atelier without copying
 * URLs by hand or trusting an image owned by another product.
 */
export async function setAdminProductLegendImageAction(
  productId: string,
  ...
  if (!image)
    redirect(
      messageUrl(
        ...
        productId,
        "mediaError",
        "تصویر انتخاب‌شده متعلق به این محصول نیست.",
      ),
    );
...
function variantData(form: FormData) {
  return {
    titleFa: text(form, "titleFa", 240, true)!,
    titleEn: text(form, "titleEn", 240, true)!,
    ...
    purityFineness: text(form, "purityFineness", 20)
      ...
      : null,
    makingChargeFixed: decimal(form, "makingChargeFixed"),
    makingChargePerGram: decimal(form, "makingChargePerGram"),
    makingChargePercent: decimal(form, "makingChargePercent"),
    artisticFee: decimal(form, "artisticFee"),
...
async function uniqueSku(sku: string | null, excluded?: string) {
  ...
  if (
    await prisma.productVariant.findFirst({
      where: { sku, ...(excluded ? { id: { not: excluded } } : {}) },
      select: { id: true },
    })
  ) {
...
export async function createAdminProductVariantAction(
  productId: string,
  localeValue: string,
  form: FormData,
): Promise<void> {
  ...
  const item = await product(productId);
  try {
    const data = variantData(form);
    await uniqueSku(data.sku);
    await prisma.$transaction(async (transaction) => {
      await transaction.productVariant.create({ data: { productId, ...data } });
    ...
    redirect(
      messageUrl(
        ...
        productId,
        "variantError",
        publicAssetError(error, "ساخت تنوع ناموفق بود."),
      ),
    );
  }
  refresh(productId, item.slug);
...
export async function updateAdminProductVariantAction(
  ...
  if (
    ...
    redirect(messageUrl(locale, productId, "variantError", "تنوع پیدا نشد."));
  try {
    const data = variantData(form);
    await uniqueSku(data.sku, variantId);
    await prisma.$transaction(async (transaction) => {
    ...
    redirect(
      messageUrl(
        ...
        productId,
        "variantError",
        publicAssetError(error, "ویرایش تنوع ناموفق بود."),
      ),
    );
  }
  refresh(productId, item.slug);
...
export async function deleteAdminProductVariantAction(
  productId: string,
  variantId: string,
  localeValue: string,
): Promise<void> {
  ...
  if (!ownedVariant) {
    redirect(
      messageUrl(
        ...
        productId,
        "variantError",
        "تنوع متعلق به این محصول نیست یا پیدا نشد.",
      ),
    );
  }
  ...
  if (await prisma.orderItem.count({ where: { variantId, productId } })) {
    await prisma.$transaction(async (transaction) => {
      ...
      if (archived.count !== 1)
        throw new AdminProductAssetError("آرشیو تنوع انجام نشد.");
      await syncProductInventory(transaction, productId);
    });
    refresh(productId, item.slug);
  ...
  await prisma.$transaction(async (transaction) => {
    ...
    if (removed.count !== 1)
      throw new AdminProductAssetError("حذف تنوع انجام نشد.");
    await syncProductInventory(transaction, productId);
  });
...
export async function deleteAdminProductAction(
  ...
  if (item._count.orderItems) {
    ...
    refresh(productId, item.slug);
    redirect(`/${locale}/admin/products?archived=ordered`);
  }
  await prisma.product.delete({ where: { id: productId } });
  await Promise.all(
    item.images.map((image) => removeStoredProductImage(image.imageUrl)),
  );
  refresh(productId, item.slug);
