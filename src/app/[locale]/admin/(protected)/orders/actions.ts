...
function publicOrderActionError(error: unknown, fallback: string): string {
  if (error instanceof OrderOperationError || error instanceof AdminOrderActionError) {
    return error.message;
  }
  console.error("[Eloria Admin Order Action] Unexpected error.", error);
  return fallback;
}
...
function text(form: FormData, key: string, max: number, required = false) {
  ...
  if (required && !value) throw new AdminOrderActionError("فیلدهای الزامی را کامل کنید.");
  if (value.length > max) throw new AdminOrderActionError("طول فیلد بیش از حد مجاز است.");
  return value || null;
}
async function session() { if (!(await hasValidAdminSession())) throw new AdminOrderActionError("نشست مدیریت منقضی شده است."); }
...
export async function transitionAdminOrderAction(orderId: string, localeValue: string, form: FormData): Promise<void> {
  ...
  catch (error) { redirect(`/${locale}/admin/orders/${orderId}?workflowError=${encodeURIComponent(publicOrderActionError(error, "تغییر وضعیت سفارش انجام نشد."))}`); }
...
export async function saveAdminShipmentAction(orderId: string, localeValue: string, form: FormData): Promise<void> {
  ...
  } catch (error) { redirect(`/${locale}/admin/orders/${orderId}?shipmentError=${encodeURIComponent(publicOrderActionError(error, "ثبت اطلاعات ارسال انجام نشد."))}`); }
  refresh(locale, orderId);
...
export async function markAdminPaymentRefundedAction(
  ...
  try {
    ...
    redirect(`/${locale}/admin/orders/${orderId}?paymentError=${encodeURIComponent(publicOrderActionError(error, "ثبت بازپرداخت انجام نشد."))}`);
  }
  refresh(locale, orderId);
