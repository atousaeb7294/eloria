...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/${locale}/order/${target}?receipt=${encodeURIComponent(receipt)}`, request.url));
  } catch (error) {
    console.error("[Eloria Payment Callback]", error);
    return NextResponse.redirect(new URL(`/${locale}/order/failed?reason=verification-failed`, request.url));
  }
}
