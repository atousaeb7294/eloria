...
export async function GET(request: NextRequest) {
  ...
  try {
    const last = (await latestSeoAutomationReport()) as
      | {
          cursor?: SeoCursor;
          performance?: unknown;
          performanceAt?: string;
          pageScan?: Awaited<ReturnType<typeof scanSeoPages>>;
        }
      | undefined;
    ...
    const needsPerformance =
      seoAutomationEnabled() &&
      (!last?.performanceAt ||
        Date.now() - Date.parse(last.performanceAt) > 86400000);
    const pageScan = seoAutomationEnabled()
      ? await scanSeoPages(last?.pageScan?.nextOffset)
      : null;
    if (pageScan && last?.pageScan?.pages) {
      const currentUrls = new Set(pageScan.pages.map((p) => p.url));
      pageScan.pages = [
        ...pageScan.pages,
        ...last.pageScan.pages.filter((p) => !currentUrls.has(p.url)),
      ].slice(0, 1000);
    }
    const performance = needsPerformance
      ? await measureSeoPerformance()
      : last?.performance || [];
    await prisma.seoAutomationRun.create({
      data: {
        report: JSON.parse(
          JSON.stringify({
            id: "SEO_AUTOPILOT_RUN",
            severity: "LOW",
            title: "گزارش خودکار سئو",
            detail: `${repairs.changed} اصلاح در آخرین نوبت`,
            action: "گزارش پنل سئو",
            checkedAt: new Date().toISOString(),
            ...repairs,
            pageScan,
            performance,
            performanceAt: needsPerformance
