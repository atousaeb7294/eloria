...
export async function POST(request: NextRequest) {
  ...
  if (!ipRate.allowed) {
    ...
    return NextResponse.json(
      ...
      {
        status: 429,
        headers: { ...headers(), "Retry-After": String(ipRate.retryAfterSeconds) },
      },
    );
  }
  ...
  if (body.channel !== undefined && body.channel !== "SMS") {
    return NextResponse.json(
      { successful: false, message: "فقط ورود پیامکی پشتیبانی می‌شود." },
  ...
  if (!isCustomerOtpChannelEnabled("SMS")) {
    return NextResponse.json(
      { successful: false, message: "سامانه پیامک ورود پیکربندی نشده است." },
      { status: 503, headers: headers() },
    );
  }
  ...
  if (typeof body.mobile !== "string") {
    return NextResponse.json(
      { successful: false, message: "شماره موبایل معتبر نیست." },
      { status: 400, headers: headers() },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        successful: false,
        message: error instanceof Error ? error.message : "شماره موبایل معتبر نیست.",
      },
      { status: 400, headers: headers() },
    );
  }
  ...
  if (!identityRate.allowed) {
    await recordSecurityEvent({
      ...
      userAgent,
      subject: mobile,
      details: { reason: "sms-rate-limit", retryAfterSeconds: identityRate.retryAfterSeconds },
      dispatchKey: mobile,
    });
    return NextResponse.json(
      ...
      {
        status: 429,
        headers: { ...headers(), "Retry-After": String(identityRate.retryAfterSeconds) },
      },
    );
  }
  ...
  try {
    ...
    if (!developmentMode) {
      ...
      if (!sms.configured || !sms.successful) {
        ...
        await recordSecurityEvent({
          ...
          userAgent,
          subject: mobile,
          details: { provider: "SMS_IR", configured: sms.configured, reason: "otp-delivery-failed" },
          dispatchKey: "sms-ir-otp-delivery",
        });
        return NextResponse.json(
          {
            successful: false,
            message: sms.configured ? "ارسال پیامک ناموفق بود. لطفاً دوباره تلاش کنید." : "سامانه پیامک ورود پیکربندی نشده است.",
          },
          { status: 503, headers: headers() },
        );
      }
    ...
    console.error("[Eloria Customer OTP] SMS request failed.", error);
    await recordSecurityEvent({
    ...
    return NextResponse.json(
      { successful: false, message: "ارسال کد ورود در حال حاضر امکان‌پذیر نیست." },
