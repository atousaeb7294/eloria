...
export function productionEnvironmentChecks(): Check[] {
  ...
  const turnstileEnabled = present("TURNSTILE_SECRET_KEY") || present("NEXT_PUBLIC_TURNSTILE_SITE_KEY");
  ...
  return [
    ...
    { key: "ELORIA_TRUST_PROXY", required: true, valid: ["true", "1"].includes(value("ELORIA_TRUST_PROXY")), message: "اعتماد به Proxy میزبان" },
...
export function assertProductionEnvironment(): void {
  ...
  if (failed.length) {
    throw new Error(`متغیرهای محیطی ناقص یا نامعتبر: ${failed.map(item => item.key).join(", ")}`);
  }
}
