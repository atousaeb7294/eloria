...
function adminPasswordPresent(minimum = 20): boolean {
  const passwordHash = value("ELORIA_ADMIN_PASSWORD_HASH");
  if (passwordHash) return /^scrypt\$[a-f0-9]{32}\$[a-f0-9]{128}$/i.test(passwordHash);
  return value("ELORIA_ADMIN_PASSWORD").length >= minimum;
}
...
export function productionEnvironmentChecks(): Check[] {
  ...
  return [
    ...
    { key: "ELORIA_EMBEDDED_METAL_SYNC_ENABLED", required: dynamicPricingEnabled, valid: !dynamicPricingEnabled || value("ELORIA_EMBEDDED_METAL_SYNC_ENABLED").toLowerCase() === "true", message: "Ø¯Ø± Ù¾Ø§Ø±Ø³â€ŒÙ¾Ú©ØŒ Ù‚ÛŒÙ…Øªâ€ŒÚ¯Ø°Ø§Ø±ÛŒ Ù¾ÙˆÛŒØ§ Ø¨Ù‡ Ù‡Ù…Ú¯Ø§Ù…â€ŒØ³Ø§Ø²ÛŒ Ø¯Ø§Ø®Ù„ÛŒ ÙØ¹Ø§Ù„ Ù†ÛŒØ§Ø² Ø¯Ø§Ø±Ø¯" },
    { key: "ELORIA_EMBEDDED_METAL_SYNC_INTERVAL_MINUTES", required: dynamicPricingEnabled, valid: !dynamicPricingEnabled || isIntegerInRange("ELORIA_EMBEDDED_METAL_SYNC_INTERVAL_MINUTES", 2, 60), message: "ÙØ§ØµÙ„Ù‡ Ù‡Ù…Ú¯Ø§Ù…â€ŒØ³Ø§Ø²ÛŒ Ù†Ø±Ø® Ø¨Ø§ÛŒØ¯ Ø¨ÛŒÙ† Û² ØªØ§ Û¶Û° Ø¯Ù‚ÛŒÙ‚Ù‡ Ø¨Ø§Ø´Ø¯" },

    { key: "ELORIA_ADMIN_USERNAME", required: true, valid: present("ELORIA_ADMIN_USERNAME", 3), message: "Ù†Ø§Ù… Ú©Ø§Ø±Ø¨Ø±ÛŒ Ù…Ø¯ÛŒØ±" },
    { key: "ELORIA_ADMIN_PASSWORD_HASH", required: true, valid: adminPasswordPresent(20), message: "Ù‡Ø´ scrypt Ù…Ø¹ØªØ¨Ø± Ù…Ø¯ÛŒØ± (Ù…ØªØºÛŒØ± Ù‚Ø¯ÛŒÙ…ÛŒ Ø±Ù…Ø² ÙÙ‚Ø· Ø¨Ø±Ø§ÛŒ Ø³Ø§Ø²Ú¯Ø§Ø±ÛŒ Ù¾Ø°ÛŒØ±ÙØªÙ‡ Ù…ÛŒâ€ŒØ´ÙˆØ¯)" },
    { key: "ELORIA_ADMIN_PASSWORD_DUPLICATE", required: false, valid: !(value("ELORIA_ADMIN_PASSWORD_HASH") && value("ELORIA_ADMIN_PASSWORD")), message: "ÙÙ‚Ø· ÛŒÚ©ÛŒ Ø§Ø² PASSWORD_HASH ÛŒØ§ PASSWORD ØªØ¹Ø±ÛŒÙ Ø´ÙˆØ¯Ø› Ù…Ù‚Ø¯Ø§Ø± Ù‡Ø´ Ø§ÙˆÙ„ÙˆÛŒØª Ø¯Ø§Ø±Ø¯" },
    ...
    { key: "ELORIA_CUSTOMER_SMS_OTP_ENABLED", required: customerAuthEnabled, valid: !customerAuthEnabled || (isExplicitBoolean("ELORIA_CUSTOMER_SMS_OTP_ENABLED") && customerSmsOtpEnabled), message: "ÙˆØ±ÙˆØ¯ Ùˆ Ø¨Ø§Ø²ÛŒØ§Ø¨ÛŒ Ø±Ù…Ø² Ù…Ø´ØªØ±ÛŒ Ø¨Ø§ÛŒØ¯ Ø¨Ø§ SMS ÙØ¹Ø§Ù„ Ø¨Ø§Ø´Ø¯" },
    { key: "SMS_IR_API_KEY", required: customerAuthEnabled || Boolean(securityAlertMobile) || Boolean(supportMobile), valid: !(customerAuthEnabled || Boolean(securityAlertMobile) || Boolean(supportMobile)) || smsApiConfigured, message: "Ú©Ù„ÛŒØ¯ API Ø³Ø±ÙˆÛŒØ³ SMS.ir" },
    { key: "SMS_IR_VERIFY_TEMPLATE_ID", required: customerAuthEnabled, valid: !customerAuthEnabled || smsVerifyConfigured, message: "Ø´Ù†Ø§Ø³Ù‡ Ù‚Ø§Ù„Ø¨ Verify Ø¯Ø± SMS.ir" },
    { key: "SMS_IR_VERIFY_PARAMETER", required: customerAuthEnabled, valid: !customerAuthEnabled || /^[A-Za-z][A-Za-z0-9_-]{0,31}$/.test(value("SMS_IR_VERIFY_PARAMETER")), message: "Ù†Ø§Ù… Ù¾Ø§Ø±Ø§Ù…ØªØ± Ù‚Ø§Ù„Ø¨ Verify Ø¯Ø± SMS.ir" },
    { key: "SMS_IR_LINE_NUMBER", required: Boolean(securityAlertMobile) || Boolean(supportMobile), valid: !(Boolean(securityAlertMobile) || Boolean(supportMobile)) || smsLineConfigured, message: "Ø´Ù…Ø§Ø±Ù‡ Ø®Ø· Ø§Ø±Ø³Ø§Ù„ SMS.ir Ø¨Ø±Ø§ÛŒ Ù‡Ø´Ø¯Ø§Ø± ÛŒØ§ Ù¾Ø´ØªÛŒØ¨Ø§Ù†ÛŒ" },
