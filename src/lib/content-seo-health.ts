...
export type ContentSeoHealth = {
  ...
  productsMissingLegend: number;
  productsMissingAtelierMedia: number;
...
export function buildContentSeoHealth(
  ...
  if (metrics.activeProductCount === 0) {
    ...
    const legendRatio =
      metrics.productsMissingLegend / metrics.activeProductCount;
    const atelierMediaRatio =
      metrics.productsMissingAtelierMedia / metrics.activeProductCount;

    score -= descriptionRatio * 32;
    score -= imageRatio * 24;
    score -= legendRatio * 18;
    score -= atelierMediaRatio * 10;
    ...
    if (metrics.productsMissingLegend > 0) {
      issues.push({
        id: "PRODUCT_LEGENDS_INCOMPLETE",
        severity: legendRatio >= 0.3 ? "HIGH" : "MEDIUM",
        title: "برخی محصولات افسانهٔ پنهانِ قابل‌پیگیری ندارند",
        detail: `${metrics.productsMissingLegend} محصول فعال، کلید یا متن افسانهٔ پنهان ندارد.`,
        action:
          "در پنل محصولات، افسانه را تولید یا بازبینی کنید تا به یکی از هفت نگهبان وصل شود.",
      });
    }

    if (metrics.productsMissingAtelierMedia > 0) {
      issues.push({
        id: "PRODUCT_ATELIER_MEDIA_INCOMPLETE",
        severity: atelierMediaRatio >= 0.5 ? "MEDIUM" : "LOW",
        title: "تصویر شخصیت یا فضای افسانهٔ بعضی محصولات کامل نیست",
        detail: `${metrics.productsMissingAtelierMedia} محصول فعال هنوز هر دو تصویر آتلیه را ندارد.`,
        action:
          "از گالری همان محصول، یک تصویر را برای «شخصیت» و یک تصویر را برای «فضای افسانه» انتخاب کنید.",
      });
    }
  }
...
export async function getContentSeoHealth(): Promise<ContentSeoHealth> {
  ...
  const productsMissingLegend = activeProducts.filter(
    (product) =>
      !nonEmpty(product.mythKey) ||
      (!nonEmpty(product.legendFa) && !nonEmpty(product.legendEn)),
  ).length;
  const productsMissingAtelierMedia = activeProducts.filter(
    (product) =>
      !nonEmpty(product.characterImageUrl) ||
      !nonEmpty(product.worldSceneImageUrl),
  ).length;
  ...
  return buildContentSeoHealth({
    ...
    productsMissingLegend,
    productsMissingAtelierMedia,
