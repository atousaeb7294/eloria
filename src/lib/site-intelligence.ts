...
export async function getSiteIntelligence() {
  ...
  const [content, finance, prices, unavailableProducts, paymentReview, watchCount, eventGroups, vitalRows, latestBriefing, productViews, productSales, customerOrderGroups, qualityProducts, currentWeekViews, previousWeekViews, currentWeekSales, marketingSources] = await Promise.all([
    getContentSeoHealth(),
  ...
  const currentWeekViewMap = new Map(
    currentWeekViews
      .filter((row) => Boolean(row.productSlug))
      .map((row) => [row.productSlug as string, row._count._all]),
  );
  const previousWeekViewMap = new Map(
    previousWeekViews
      .filter((row) => Boolean(row.productSlug))
      .map((row) => [row.productSlug as string, row._count._all]),
  );
  const currentWeekSalesMap = new Map(
    currentWeekSales.map((row) => [row.productSlug, row._sum.quantity ?? 0]),
  );
  const momentumSlugs = new Set([
    ...currentWeekViewMap.keys(),
    ...previousWeekViewMap.keys(),
  ]);
  const merchandisingMomentum = Array.from(momentumSlugs)
    .map((slug) => {
      const currentViews = currentWeekViewMap.get(slug) ?? 0;
      const previousViews = previousWeekViewMap.get(slug) ?? 0;
      const rawGrowth = previousViews === 0
        ? currentViews > 0 ? 100 : 0
        : ((currentViews - previousViews) / previousViews) * 100;
      const growthPercent = Math.round(Math.max(-100, Math.min(999, rawGrowth)) * 10) / 10;
      return {
        slug,
        currentViews,
        previousViews,
        growthPercent,
        sales: currentWeekSalesMap.get(slug) ?? 0,
      };
    })
    .filter((item) => item.currentViews >= 3 || item.sales > 0)
    .sort((left, right) =>
      right.growthPercent - left.growthPercent ||
      right.currentViews - left.currentViews ||
      right.sales - left.sales,
    )
    .slice(0, 6);

  const risingProduct = merchandisingMomentum.find(
    (item) => item.currentViews >= 8 && item.growthPercent >= 80,
  );
  if (risingProduct) {
    actions.push({
      id: `momentum-${risingProduct.slug}`,
      priority: "low",
      title: "یک اثر شتاب توجه گرفته است",
      detail: `${risingProduct.slug} در ۷ روز اخیر ${risingProduct.currentViews} بازدید داشته و نسبت به هفته قبل ${risingProduct.growthPercent}٪ رشد توجه نشان می‌دهد؛ جایگاه آن در Home و موجودی را بررسی کنید.`,
      href: `/fa/products/${risingProduct.slug}`,
      hrefLabel: "مشاهده اثر",
    });
  }
  ...
  return {
    ...
    marketing: {
      ...
      momentum: merchandisingMomentum,
      topViewedProducts,
      topSellingProducts,
