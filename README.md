...
- ورود مشتری با شماره موبایل و رمز عبور انجام می‌شود.
- عضویت، ورود جایگزین، فراموشی رمز و بازیابی رمز با پیامک Verify سرویس SMS.ir انجام می‌شود.
- در پنل ورود و پروفایل مشتری هیچ ورودی ایمیل وجود ندارد؛ ایمیل Checkout فقط برای اطلاعات تماس سفارش و مستقل از احراز هویت است.
- در دسکتاپ، متن فارسی سایت با فونت خودمیزبان ایران نستعلیق نمایش داده می‌شود؛ در موبایل برای خوانایی و شکل‌دهی درست فارسی، Vazirmatn فعال است.

برای فعال‌سازی واقعی، مقدارهای `SMS_IR_API_KEY`، `SMS_IR_VERIFY_TEMPLATE_ID` و `SMS_IR_VERIFY_PARAMETER` را در `.env` تنظیم کنید. راهنمای کامل در `ELORIA_FINAL_INSTALL_FA.md` است.
