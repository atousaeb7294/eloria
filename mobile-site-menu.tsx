...
export function MobileSiteMenu() {
  ...
  const storyHref = `/${locale}/story`;

  const atelierHref = `/${locale}/atelier`;
  ...
  const isStoryActive = pathname.startsWith(`/${locale}/story`);

  const isAtelierActive = pathname.startsWith(`/${locale}/atelier`);
  ...
  const primaryLinks = [
    ...
    },
    {
      ...
      description: isPersian
        ? "افسانهٔ مادر، پیمان نخستین و هفت نگهبان"
        : "The mother legend, First Covenant and Seven Guardians",
      href: storyHref,
      active: isStoryActive,
      icon: BookOpenText,
    },
    {
      label: isPersian ? "آتلیهٔ الوریا" : "The Eloria Atelier",
      description: isPersian
        ? "شخصیت‌ها، نگهبانان و افسانه‌های پنهان"
        : "Characters, Guardians and hidden legends",
      href: atelierHref,
      active: isAtelierActive,
      icon: Sparkles,
    },
    {
      label: isPersian ? "درباره ما" : "About Eloria",
      description: isPersian
        ? "معرفی رسمی برند و زیورهای دست‌ساز"
