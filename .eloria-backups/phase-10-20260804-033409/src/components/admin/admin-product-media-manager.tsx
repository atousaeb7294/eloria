  ...
  return <section className="overflow-hidden rounded-[26px] border border-[#d0b359]/16 bg-[#041d15]/82">
    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      {images.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{images.map(image => <article key={image.id} className="overflow-hidden rounded-2xl border border-[#d0b359]/14 bg-[#02150f]/80">
        <div className="relative aspect-[4/3] bg-black/20">{/* eslint-disable-next-line @next/next/no-img-element */}<img alt={image.altFa || "تصویر محصول"} className="h-full w-full object-cover" src={image.imageUrl}/>{image.isPrimary ? <span className="absolute start-3 top-3 inline-flex items-center gap-1 rounded-full bg-[#061a12]/90 px-2.5 py-1 text-[11px] text-[#f0d478]"><Star className="size-3 fill-current"/>اصلی</span> : null}</div>
