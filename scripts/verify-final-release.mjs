...
check("هوشمندی افسانه به API پولی وابسته نیست", has("src/lib/ai/myth-generator.ts", 'return "template"') && has("src/lib/ai/myth-generator.ts", 'selectedProvider === "ollama"') && has(".env.example", 'ELORIA_AI_PROVIDER="template"'));
...
check("خانه آثار بیشتری را با چیدمان متحرک نمایش می‌دهد", has("src/app/api/home-featured-products/route.ts", "SMART_RESULT_LIMIT = 12") && has("src/components/home-featured-album.tsx", "TAPESTRY_LAYOUTS") && has("src/components/home-featured-album.tsx", "eloria-product-drift"));
check("CTA خانه به جست‌وجو در تمام آثار تغییر کرده است", has("src/components/hero-showcase.tsx", "جست‌وجو در تمام آثار") && has("src/components/home-featured-album.tsx", "جست‌وجو در تمام آثار"));
